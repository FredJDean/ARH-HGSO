...
from util.data import load_FreeBase_data
...
def main(args):
    ...
    # 只适用于同质图
    features, type_mask, labels, train_idx, val_idx, test_idx, homo_graph, homo_dhg_graph, semantic_graph, hp_G \
        = load_FreeBase_data(device=args['device'])
    labels_plt = labels
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='FreeBase', choices=['DBLP', 'DBLP_S', 'ACM_NEW', 'ACM', 'IMDB', 'ACM_L', 'MAG', 'FreeBase'])
    ...
    parser.add_argument('--loss_alpha', type=float, default=0.05, help='loss weight of unsupervised')
