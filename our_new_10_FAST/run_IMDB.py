...
from util.data import load_IMDB_data
...
def main(args):
    ...
    features, type_mask, labels, train_val_test_idx, homo_graph, homo_dhg_graph, meta_data_e, semantic_graph, hp_G \
        = load_IMDB_data(device=args['device'])
    ...
    for cur_repeat in range(args['repeat']):
        ...
        for epoch in range(args['num_epochs']):
            emb, logits, loss_unsupervised, _, _, att_meta, att_relation = model(features, homo_dhg_graph, homo_graph, hp_G, type_mask)
        ...
        with torch.no_grad():
            ...
            # scipy.sparse.save_npz("relation_imdb", relation_A)
            # scipy.sparse.save_npz("meta_path_imdb", meta_path_A)
            print("meta path att:", att_meta)
            print("relation att:", att_relation)
            print(test_macro_f1)
            print(test_micro_f1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='IMDB', choices=['DBLP', 'DBLP_S', 'ACM_NEW', 'ACM', 'IMDB', 'ACM_L'])
    ...
    parser.add_argument('--save-postfix', default='IMDB', help='Postfix for the saved model and result. Default is ACM.')
