...
from util.data import load_YELP_data
...
def main(args):
    ...
    features, type_mask, labels, train_val_test_idx, homo_graph, homo_dhg_graph, meta_data_e, semantic_graph, hp_G \
        = load_YELP_data(device=args['device'])
    labels_plt = labels
    ...
    for cur_repeat in range(args['repeat']):
        ...
        with torch.no_grad():
            ...
            # relation_A = scipy.sparse.csr_matrix(relation_A.cpu())
            # meta_path_A = scipy.sparse.csr_matrix(meta_path_A.cpu())
            # scipy.sparse.save_npz("relation_yelp", relation_A)
            # scipy.sparse.save_npz("meta_path_yelp", meta_path_A)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='YELP', choices=['DBLP', 'DBLP_S', 'ACM_NEW', 'ACM', 'IMDB', 'ACM_L'])
    ...
    parser.add_argument('--save-postfix', default='YELP', help='Postfix for the saved model and result. Default is ACM.')
