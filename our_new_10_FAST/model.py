...
class HGFS(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_classes, dataset, len_sub_graph, len_homo_graph, len_relation_graph,
        ...
        self.relu = nn.ReLU()
        # self.gcn_meta_path = AR_GCN(hidden_dim, hidden_dim, alpha=alpha)
        # self.gcn_relation = AR_GCN(hidden_dim, hidden_dim, alpha=alpha)
        ...
        # yelp 0.3 ACM 1.5 IMDB 1.5 DBLP: 0.9
        # IMDB 2.0 0.9
        # 主要调这个
        self.contrast = Contrast(hidden_dim, 1.5, 0.9)
    ...
    def forward(self, features_list, sub_graph, homo_graph, hp_G, type_mask):
        ...
        # emb_relation = self.gcn_relation(embedding_all, r_g.indices(), r_g.values())[0:num]
        # emb_meta_path = self.gcn_meta_path(embedding, s_g.indices(), s_g.values())
        emb_relation = self.gcn_relation(embedding_all, r_g)[0:num]
        emb_meta_path = self.gcn_meta_path(embedding, s_g)
